...
//-----------------------------------------------------------------------------
{
  ...
  // For pktR stats and UCS
  eNB_MAC_INST *mac = &eNB_mac_inst[ctxt_pP->module_id];
  //int g_count = 0;
  ...
  if (MBMS_flagP == 0 ) {
    if (srb_flagP) { //SRB1/2
      ...
      if (pdcp_p->seq_num_size == PDCP_SN_7BIT) {
        ...
        ///////////////////
        mac->eNB_stats[0].seqnum_long[pdcp_rx_sn_index[ctxt_pP->rnti]][ctxt_pP->rnti] = sequence_number; //pktR
	      printf("*****RX SEQUENCE NUMBER LONG STRUCT [UE %x] ==> (%d)\n", ctxt_pP->rnti, mac->eNB_stats[0].seqnum_long[pdcp_rx_sn_index[ctxt_pP->rnti]][ctxt_pP->rnti]);
	      if (pdcp_rx_sn_index[ctxt_pP->rnti] != 999) {
	        pdcp_rx_sn_index[ctxt_pP->rnti]++;
          //mac->eNB_stats[0].seqnum_count[ctxt_pP->rnti] = g_count++;
	      }
        if(pdcp_rx_sn_index[ctxt_pP->rnti] == 999) {
          //mac->eNB_stats[0].seqnum_count[ctxt_pP->rnti] = 0;
          pdcp_rx_sn_index[ctxt_pP->rnti] = 0;
        }
